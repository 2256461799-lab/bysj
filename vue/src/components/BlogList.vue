<template>
  <div>
    <el-card class="card" style="min-height: 80vh">
      <div class="blog-box" v-for="item in tableData" :key="item.id" v-if="total > 0">
        <div style="flex: 1; width: 0">
          <div class="blog-title" @click="$router.push('/front/blogDetail?blogId=' + item.id)">{{ item.title }}</div>
          ...
          <div style="display: flex; align-items: center">
            <div style="flex: 1; font-size: 13px">
              ...
              <span v-if="showOpt" style="margin-left: 10px; color: #1d3557; cursor: pointer" @click="editBlog(item.id)"><i class="el-icon-edit"></i>编辑</span>
            </div>
    ...
    loadBlogs(pageNum) {
      ...
      this.$request.get(url, {
        params: {
          ...
          categoryName: this.categoryName === '全部日志' ? null : this.categoryName,
